...
function menuKur() {
  ...
  /* Kaydirmayi TARAYICININ kendi yumusak kaydirmasiyla (CSS scroll-behavior) tek seferde yapiyoruz.
     Elle animasyon (setInterval + scrollTo) 3D acilisiyla ayni ana denk gelince titreme yapiyordu (omer, 12.09);
     ayrica fonksiyon iki kez tetiklenip iki animasyon cakisiyordu -> tek cagri + kilit. */
  var mgBekle = null;
  function mobilGoster() {
    if (!MOBIL()) return;
    if (mgBekle) return;                         /* ust uste cagrilari tek sefere indir */
    mgBekle = setTimeout(function () {
      mgBekle = null;
      if (!kap || kap.style.display === 'none') return;
      var r = kap.getBoundingClientRect();
      if (!r.height) return;
      if (r.top >= 0 && r.top < window.innerHeight * 0.45) return;   /* kare zaten ekranda */
      var hedef = Math.max(0, Math.round(window.scrollY + r.top - 64));   /* sabit baslik payi */
      var kok = document.documentElement, eski = kok.style.scrollBehavior;
      kok.style.scrollBehavior = 'smooth';
      window.scrollTo(0, hedef);
      setTimeout(function () { kok.style.scrollBehavior = eski; }, 800);
    }, 240);                                     /* once 3D acilsin, duzen otursun */
  }
  function modSec(uc) {
    if (!!uc === acik) return;
