/* ===== NAZZE EV ORGANIZATOR — canli modul (12.09.2026) =====
   ...
   1) URUN SAYFASI (slug urunler.json'da): renk/parca/model secici + 3D onizleme + "Urun Ozellikleri" (olcu, sema, hikaye)
      + fiyat notu + MagSafe baslik istisnasi + gizli "Renk Seçimi" ozel alani (siparise otomatik islenir).
...
function menuKur() {
  ...
  function ekle() {
    try {
      ...
      /* mobil: SADECE Magaza alt listesi. Ust seviyede ayri madde YOK — mobil menude "Ev Organizator"
         iki kere gorunuyordu (omer, 12.09); masaustunde ayri sekme olarak kaliyor. */
      ...
      /* onceki surumun ekledigi ust seviye maddeyi (varsa) temizle */
      var eskiMli = document.getElementById('nz-evo-mli'); if (eskiMli) eskiMli.parentNode.removeChild(eskiMli);
  ...
  function urunHazirla(u, veri) {
    ...
    if (!document.getElementById('nz-evurun-stil')) {
      ...
      st.textContent = '.nz-hikaye{margin-bottom:34px}.nzC{display:grid;grid-template-columns:1fr 1fr;gap:4px 64px}.nzC .r{display:flex;align-items:center;gap:16px;padding:11px 0;border-bottom:1px dashed #e0dacc}.nzC .r svg{width:22px;height:22px;stroke:#2b2a26;fill:none;stroke-width:1.4;flex-shrink:0;opacity:.75}.nzC .r .t{font-size:.66rem;letter-spacing:.16em;color:#8a8378;width:74px;flex-shrink:0}.nzC .r .v{font-size:.86rem;font-weight:600;color:#2b2a26}.nz-eko{margin-top:18px;font-size:.74rem;color:#7a7466;display:flex;align-items:center;gap:8px}.nz-eko svg{width:15px;height:15px;stroke:#7a9471;fill:none;stroke-width:1.6;flex-shrink:0}@media (max-width:900px){.nzC{grid-template-columns:1fr}}' +
        ...
        '#ev-secenekler{margin:6px 0 16px;list-style:none;padding:0}' +
    ...
    if (!document.getElementById('ev-secenekler')) {
      /* lamba duzeni: baslik > RENK SECENEKLERI > fiyat > ozet > dugmeler. Tema, lambalarda variant-box'i ul.product-profile-info icinde
         fiyat satirinin ONUNE tasiyor; biz de secenek kutusunu fiyat li'sinin onune koyuyoruz. */
      var info = pp.querySelector('ul.product-profile-info'), vb = pp.querySelector('.variant-box'), pb = pp.querySelector('.product-buttons');
      var fiyatLi = info ? [].slice.call(info.children).filter(function (li) { return li.querySelector('.sale-price') || /(TL|₺)\s*$/.test(li.textContent.trim()); })[0] : null;
      var kutu = document.createElement(fiyatLi ? 'li' : 'div'); kutu.id = 'ev-secenekler';
      if (fiyatLi) info.insertBefore(kutu, fiyatLi); else if (info) info.insertAdjacentElement('afterend', kutu); else if (vb) vb.parentNode.insertBefore(kutu, vb); else if (pb) pb.parentNode.insertBefore(kutu, pb); else pp.appendChild(kutu);
    }
    /* fiyat notu (setler) */
  ...
  function secenekKur() {
    ...
    kutu.addEventListener('click', function (e) {
      ...
      /* model secimi: 3D'yi SADECE mobilde otomatik ac (masaustunde varyant degisimi galeri yuksekligini oynatip sayfayi kaydiriyordu) */
      if (vb) { varyantSec(+vb.dataset.v); if (MOBIL() && !acik) modSec(true); mobilGoster(); return; }
      var pb = e.target.closest('.ev-parca');
      if (pb) { aktif = +pb.dataset.i; guncelle(); return; }
      var rb = e.target.closest('.ev-renk');
      if (rb) { secim[aktif] = rb.dataset.renk; renkSecildi = true; guncelle(); boya(aktif); if (!acik) modSec(true); mobilGoster(); }
    });
    guncelle();
    return true;
  }
  ...
  /* Qukasoft ozel alani: addCart() div[data-product-special-fields=ID] icindeki .field-input'lari special_field[ad]=deger olarak sepete gonderir.
     Canlida "Renk Seçimi" ozel alani panelde tanimlanir; tema kutuyu basarsa doldururuz, basmazsa gizli girdi olusturup doldururuz. */
  function ozelAlanYaz() {
    ...
    /* ozel alanin paneldeki adi "Renk Secimi" (omer, 12.09; eski adi "Renk Kombinasyonu" yedek olarak kaliyor) */
    var inp = div.querySelector('.field-input[data-label="Renk Seçimi"]') || div.querySelector('.field-input[data-label="Renk Kombinasyonu"]') || div.querySelector('input.field-input[type="text"], textarea.field-input, input.field-input:not([type=file]):not([type=checkbox]):not([type=radio])');
    if (!inp) { inp = document.createElement('input'); inp.type = 'hidden'; inp.className = 'field-input'; inp.name = 'Renk Seçimi'; inp.setAttribute('data-required', '0'); div.appendChild(inp); }
  ...
  function MOBIL() { return window.innerWidth < 992; }
  /* Mobilde 3D karesi secenek kutusunun USTUNDE kaldigi icin renk secince sonuc gorunmuyordu (omer, 12.09):
     renk/model secilince 3D karesini yumusakca ekrana getir. Zaten yeterince goruniyorsa dokunma (surekli ziplamasin). */
  /* Not: bu temada window.scrollTo({behavior:'smooth'}) calismiyor (sessizce yok sayiliyor),
     duz scrollTo calisiyor. Bu yuzden yumusak kaydirmayi kendimiz animasyonluyoruz. */
  function kaydir(hedef) {
    hedef = Math.max(0, Math.round(hedef));
    var bas = window.scrollY || document.documentElement.scrollTop || 0;
    var mesafe = hedef - bas;
    if (Math.abs(mesafe) < 4) return;
    /* setTimeout tabanli: requestAnimationFrame sekme arka plandayken durur */
    var sure = Math.min(450, Math.max(240, Math.abs(mesafe))), t0 = Date.now(), iv;
    iv = setInterval(function () {
      var p = Math.min(1, (Date.now() - t0) / sure);
      var e = p < 0.5 ? 2 * p * p : 1 - Math.pow(-2 * p + 2, 2) / 2;   /* easeInOutQuad */
      window.scrollTo(0, Math.round(bas + mesafe * e));
      if (p >= 1) clearInterval(iv);
    }, 16);
  }
  function mobilGoster() {
    if (!MOBIL()) return;
    function git() {
      if (!kap || kap.style.display === 'none') return;   /* gizliyken rect 0 doner, olcme */
      var r = kap.getBoundingClientRect();
      if (!r.height) return;
      if (r.top >= 0 && r.top < window.innerHeight * 0.45) return;   /* karenin ust kenari zaten ekranda */
      kaydir(window.scrollY + r.top - 64);                            /* sabit baslik payi */
    }
    /* 3D karesi yeni aciliyorsa duzen once otursun, sonra olcup kaydiralim */
    setTimeout(git, 80);
    setTimeout(git, 260);
  }
  function modSec(uc) {
    if (!!uc === acik) return;
    /* Galeri (dikey foto listesi) ile 3D karesinin yukseklikleri cok farkli: mod degisince sayfa
       kisalip/uzayip kullaniciyi Urun Ozellikleri'ne atiyordu (omer, 12.09). Secenek kutusunun
       ekrandaki konumunu sabit tutup farki scroll ile telafi ediyoruz. */
    var ref = document.getElementById('ev-secenekler') || kap;
    var refOnce = ref ? ref.getBoundingClientRect().top : null;
    function hizala() {
      if (MOBIL()) return;   /* mobilde 3D karesine yumusak scroll var (mobilGoster), telafiye gerek yok */
      if (refOnce == null || !ref) return;
      var fark = ref.getBoundingClientRect().top - refOnce;
      if (Math.abs(fark) > 1) window.scrollBy(0, fark);
    }
    acik = !!uc;
    ...
    hizala();
    setTimeout(hizala, 16);
    setTimeout(hizala, 80);
  }
